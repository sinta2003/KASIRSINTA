<?php
    ...
    if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
        $insertSQL = sprintf("UPDATE produk SET kodeproduk=%s, kategori=%s, namaproduk=%s, hargadasar=%s, hargajual=%s, stok=%s  WHERE idproduk=%s",
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1">
    ...
    Pilih Kategori : <select name="kategori">
        <?php do { ?>
        <option value="<?php echo $rowKategori['idkategori']; ?>" 
        <?php if (!(strcmp($rowKategori['idkategori'], htmlentities($rowProduk['kategori'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>
